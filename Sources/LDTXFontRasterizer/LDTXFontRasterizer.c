...
int32_t ldtx_bake_clock_ascii_glyphs(const uint8_t *font_data, float pixel_height, uint8_t *atlas_pixels,
  ...
  if (font_data == NULL || atlas_pixels == NULL || glyphs == NULL || glyph_count < required_glyph_count ||
      atlas_width <= 0 || atlas_height <= 0 || pixel_height <= 0.0f) {
    return 0;
  }
  ...
  for (int32_t index = 0; index < required_glyph_count; index++) {
    ...
    if (ldtx_clock_uses_code_point(code_point)) {
      sdf = stbtt_GetCodepointSDF(&font, scale, code_point, padding, 128, 8.0f, &width, &height, &xoff, &yoff);
    }
    ...
    if (sdf != NULL && width <= cell_width && height <= cell_height) {
      for (int row = 0; row < height; row++) {
        memcpy(atlas_pixels + (cell_y + row) * atlas_width + cell_x, sdf + row * width, (size_t)width);
      }
