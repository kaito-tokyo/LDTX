...
public struct VisionModelSettingsView: View {
    ...
    private func refreshDownloadedModels() {
        downloadedRepositoryIDs = Set(models.compactMap { option in
            service.isDownloaded(model: option.model) ? option.model.repositoryID : nil
        })
    }
