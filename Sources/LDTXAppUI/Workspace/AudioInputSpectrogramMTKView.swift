...
final class AudioInputSpectrogramMTKView: MTKView, MTKViewDelegate {
  ...
  func draw(in view: MTKView) {
    if !hasLoggedDrawFailure {
      if device == nil {
        ...
        audioInputSpectrogramViewLogger.error(
          "Spectrogram draw skipped: renderPassDescriptor is nil")
    ...
    Self.quadVertices.withUnsafeBytes { buffer in
      ...
      encoder.drawPrimitives(
        type: .triangle, vertexStart: 0, vertexCount: Self.quadVertices.count / 4)
    }
    encoder.endEncoding()
  ...
  private func commonInit() {
    ...
    pipelineState =
      Self.sharedMetalResources.pipelineState
      ?? Self.makePipelineState(device: device, pixelFormat: colorPixelFormat)
    samplerState = Self.sharedMetalResources.samplerState ?? Self.makeSamplerState(device: device)
    delegate = self
    wantsLayer = true
  ...
  private func updateTexture() {
    ...
    guard let spectrogramTexture,
      snapshot.pixels.count == width * height
    else {
      return
    }
  ...
  nonisolated private static func makePipelineState(
    ...
    do {
      ...
      guard let vertexFunction = library.makeFunction(name: "audio_input_spectrogram_vertex"),
        let fragmentFunction = library.makeFunction(name: "audio_input_spectrogram_fragment")
      else {
        audioInputSpectrogramViewLogger.error(
          "Spectrogram shader functions were not found in library")
        return nil
      }
