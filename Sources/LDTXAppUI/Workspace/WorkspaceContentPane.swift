...
enum WorkspaceResourcePreviewFactory {
    static func inputDeviceComposite(_ inputDevice: WorkspaceInputDeviceRecord) -> CompositeProgramDefinition {
...
struct WorkspaceInputDevicePreviewPane: View {
    ...
    var body: some View {
        Group {
                ...
                if let physicalDeviceID = inputDevice.physicalDeviceID {
                    VStack(alignment: .leading, spacing: 12) {
                        Text("\(inputDevice.name) Preview")
                            .font(.headline)
                        AudioInputSpectrogramPane(audioDeviceID: physicalDeviceID)
                    }
                    .padding()
                } else {
                    ContentUnavailableView(
            ...
            case .unspecified:
                ContentUnavailableView("Unsupported Input Device", systemImage: "questionmark.square.dashed")
            }
        }
