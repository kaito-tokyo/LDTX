...
struct WorkspaceVideoComponentDetailPane: View {
    ...
    var body: some View {
        Form {
            if let component = selectedComponentBinding {
                ...
                if component.wrappedValue.component.definition.usesInputCameraDevice {
                    Section("Crop") {
                        cropField("Top", value: component.sourceCropTop, accessibilityIdentifier: "videoComponentCropTopField")
                        cropField("Right", value: component.sourceCropRight, accessibilityIdentifier: "videoComponentCropRightField")
                        cropField("Bottom", value: component.sourceCropBottom, accessibilityIdentifier: "videoComponentCropBottomField")
                        cropField("Left", value: component.sourceCropLeft, accessibilityIdentifier: "videoComponentCropLeftField")
                    }
    ...
    private var selectedComponentBinding: Binding<WorkspaceVideoComponentRecord>? {
        guard case .videoComponent(let id) = selectedSidebarItem,
              let index = videoComponents.firstIndex(where: { $0.id == id }) else { return nil }
    ...
    private var componentStepBinding: Binding<CompositeProgramStep>? {
        ...
        return Binding(
            get: {
                CompositeProgramStep(displayName: component.wrappedValue.name, component: component.wrappedValue.component)
            },
