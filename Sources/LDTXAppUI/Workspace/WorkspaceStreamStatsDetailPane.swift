    ...
    VStack(alignment: .leading, spacing: 0) {
      ...
      Form {
        Section("Canvas Preset") {
          ...
          Picker("Video Bit Rate", selection: $outputCanvas.videoBitRate) {
            ForEach([4_500_000, 6_000_000, 9_000_000], id: \.self) { bitRate in
              Text(verbatim: String(format: "%.1f Mbps", Double(bitRate) / 1_000_000))
                .tag(bitRate)
            }
          }
          .pickerStyle(.menu)
