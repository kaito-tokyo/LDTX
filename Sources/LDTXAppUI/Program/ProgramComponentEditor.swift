...
struct ProgramAudioChannelDefinitionEditor: View {
    @Binding var channel: ProgramAudioChannel
    var workspaceInputDevices: [WorkspaceInputDeviceRecord]
    var canMoveUp: Bool
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Picker("Audio Component", selection: audioChannelDefinitionBinding) {
                    ForEach(ProgramAudioChannelDefinition.allCases) { definition in
                        Text(audioChannelLabel(for: definition)).tag(definition)
                    }
                }
            ...
            audioChannelParameterControls
        }
        .padding(.vertical, 6)
    }

    @ViewBuilder
    private var audioChannelParameterControls: some View {
        switch componentBinding.wrappedValue {
        case .inputAudioDevice:
            audioInputDeviceControl(payload: inputAudioDeviceBinding)
    ...
    private var componentBinding: Binding<ProgramAudioChannelComponent> {
        $channel.component
    }

    private var audioChannelDefinitionBinding: Binding<ProgramAudioChannelDefinition> {
        Binding(
            get: { channel.component.definition },
            set: { newValue in
                channel.component = .defaultComponent(for: newValue)
            }
        )
    }

    private func audioChannelLabel(for definition: ProgramAudioChannelDefinition) -> String {
        switch definition {
        case .inputAudioDevice:
    ...
    private var inputAudioDeviceBinding: Binding<InputAudioDeviceComponent> {
        Binding(
            get: {
                if case let .inputAudioDevice(payload) = componentBinding.wrappedValue {
                    return payload
                }
                return InputAudioDeviceComponent()
            },
            set: { componentBinding.wrappedValue = .inputAudioDevice($0) }
        )
    }
