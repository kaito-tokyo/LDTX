...
final class AudioPeakMeterMTKView: MTKView, MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        ...
        let measuredDecibels = rawPeak > 0
            ? 20 * log10(rawPeak)
        ...
        let displayDecibels = min(max(displayedDecibels, MeterScale.minimumDecibels), MeterScale.maximumDecibels)
    ...
    private static func makePipelineState(
        ...
        do {
            ...
            guard let vertexFunction = library.makeFunction(name: "audio_peak_meter_vertex"),
                  let fragmentFunction = library.makeFunction(name: "audio_peak_meter_fragment") else {
                return nil
            }
