...
public struct RecordingShieldVerificationResult: Codable, Equatable, Sendable {
  ...
  public init(status: Status, issues: [RecordingShieldIssue] = [], reason: RecordingShieldUnverifiableReason? = nil) {
    self.status = status; self.issues = issues; self.reason = reason
  }
}
...
public struct RecordingShieldVerifier: Sendable {
  ...
  public func verify(packageAt root: URL) -> RecordingShieldVerificationResult {
    ...
    let manifestDescriptor = openat(rootDescriptor, RecordingShieldProfile.manifestFileName, O_RDONLY | O_NONBLOCK | O_NOFOLLOW)
    guard manifestDescriptor >= 0 else {
      if errno == ENOENT { return .init(status: .unverifiable, reason: .noShield) }
      if errno == ELOOP {
        return .init(status: .invalid, issues: [
          .init(kind: .unsafeEntry, path: RecordingShieldProfile.manifestFileName, message: "Shield manifest must be a regular file and must not be a symbolic link.")
        ])
      }
      var entryStat = stat()
      if fstatat(rootDescriptor, RecordingShieldProfile.manifestFileName, &entryStat, AT_SYMLINK_NOFOLLOW) == 0,
        (entryStat.st_mode & S_IFMT) != S_IFREG {
        return .init(status: .invalid, issues: [
          .init(kind: .unsafeEntry, path: RecordingShieldProfile.manifestFileName, message: "Shield manifest must be a regular file.")
        ])
      }
    ...
    guard manifestStat.st_size <= RecordingShieldProfile.maximumManifestSize else {
      close(manifestDescriptor)
      return .init(status: .invalid, issues: [
        .init(kind: .malformedManifest, path: RecordingShieldProfile.manifestFileName, message: "Shield manifest exceeds the maximum supported size.")
      ])
    }
    let data: Data
    do {
    ...
    do { statement = try RecordingShieldCodec.decode(data) }
    catch {
      return .init(status: .invalid, issues: [.init(kind: .malformedManifest, message: "Shield JSON does not match the statement schema.")])
    }
    ...
    if p.packageRoot != RecordingShieldProfile.packageRoot || p.digestAlgorithm != RecordingShieldProfile.digestAlgorithm
      || p.pathPolicy != RecordingShieldProfile.pathPolicy || p.entryPolicy != RecordingShieldProfile.entryPolicy
      || p.verificationPolicy != RecordingShieldProfile.verificationPolicy {
      issues.append(.init(kind: .policyMismatch, message: "Shield policy values do not exactly match Recording Shield v1."))
    }

    var listed: [String: String] = [:], normalized = Set<String>(), folded = Set<String>()
    var priorName: String?
    for subject in statement.subject {
      let name = subject.name
      if RecordingShieldEntries.pathProblem(name) {
        issues.append(.init(kind: .malformedManifest, path: name, message: "Subject path is not canonical.")); continue
      }
      if let priorName, !Array(priorName.utf8).lexicographicallyPrecedes(Array(name.utf8)) {
        issues.append(.init(kind: .malformedManifest, path: name, message: "Subjects are not uniquely ordered by UTF-8 bytes."))
      }
      priorName = name
      let fold = name.folding(options: [.caseInsensitive], locale: Locale(identifier: "en_US_POSIX"))
      if !normalized.insert(name.precomposedStringWithCanonicalMapping).inserted || !folded.insert(fold).inserted {
        issues.append(.init(kind: .pathConflict, path: name, message: "Subject path has an NFC or case-fold collision."))
      }
      let digest = subject.digest.sha256
      if digest.count != 64 || digest.contains(where: { !("0"..."9").contains(String($0)) && !("a"..."f").contains(String($0)) }) {
        issues.append(.init(kind: .malformedManifest, path: name, message: "SHA-256 digest must be 64 lowercase hexadecimal digits.")); continue
      }
      listed[name] = digest
    }
    ...
    do {
      ...
      issues.append(.init(kind: .unsafeEntry, path: path.isEmpty ? nil : path, message: "Package entry name is not valid UTF-8."))
      return .init(status: .invalid, issues: issues)
    }
    catch { return .init(status: .unverifiable, issues: issues, reason: .ioFailure) }
    var actualRegular = Set<String>(), actualNFC = Set<String>(), actualFolded = Set<String>()
    for entry in entries {
      if RecordingShieldEntries.pathProblem(entry.path) {
        issues.append(.init(kind: .unsafeEntry, path: entry.path, message: "Package entry path is not canonical."))
      }
      let actualNormalized = entry.path.precomposedStringWithCanonicalMapping
      let actualFold = actualNormalized.folding(options: [.caseInsensitive], locale: Locale(identifier: "en_US_POSIX"))
      if !actualNFC.insert(actualNormalized).inserted || !actualFolded.insert(actualFold).inserted {
        issues.append(.init(kind: .pathConflict, path: entry.path, message: "Package path has an NFC or case-fold collision."))
      }
      switch entry.kind {
      case .unsafe:
        issues.append(.init(kind: .unsafeEntry, path: entry.path, message: "Package entry is not a regular file or directory."))
      ...
      case .regular:
        ...
        guard let expected = listed[entry.path] else {
          issues.append(.init(kind: .unexpected, path: entry.path, message: "Regular file is not listed by the Shield.")); continue
        }
        guard let actual = entry.sha256 else {
          return .init(status: .unverifiable, issues: issues, reason: .ioFailure)
        }
        if actual != expected {
          issues.append(.init(kind: .modified, path: entry.path, message: "SHA-256 digest does not match."))
        }
      }
    }
    ...
    if (fstatat(rootDescriptor, RecordingPackage.finalizedMarkerFileName, &finalizedStat, AT_SYMLINK_NOFOLLOW) != 0
      ...
      issues.append(.init(kind: .missing, path: RecordingPackage.finalizedMarkerFileName, message: "A Shielded recording package requires a listed zero-byte regular .finalized marker."))
    }
