...
public enum RecordingShieldSealingError: Error, LocalizedError, Equatable, Sendable {
  case packageNotFinalized, invalidFinalizedMarker, shieldAlreadyExists
  ...
  public var errorDescription: String? {
    ...
    case .invalidFinalizedMarker:
      "Recording package finalization marker is not a zero-byte regular file."
...
public struct RecordingShieldSealer: Sendable {
  ...
  @discardableResult public func seal(packageAt root: URL) throws -> RecordingShieldStatement {
    ...
    let lockURL = root.deletingLastPathComponent().appendingPathComponent(
      ".\(root.lastPathComponent).shield.lock")
    ...
    guard lstat(finalized.path, &finalizedStat) == 0 else {
      throw RecordingShieldSealingError.packageNotFinalized
    }
    ...
    guard !fm.fileExists(atPath: output.path) else {
      throw RecordingShieldSealingError.shieldAlreadyExists
    }
    ...
    let value = RecordingShieldStatement(subject: snapshotSubjects)
    let data = try RecordingShieldCodec.encode(value)
    guard data.count <= RecordingShieldProfile.maximumManifestSize else {
      throw CocoaError(.fileWriteUnknown)
    }
    let temporary = root.appendingPathComponent(".shield.json.tmp.\(UUID().uuidString)")
    let descriptor = open(
      temporary.path, O_WRONLY | O_CREAT | O_EXCL, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)
    ...
    do {
      ...
      try handle.close()
      guard renameatx_np(AT_FDCWD, temporary.path, AT_FDCWD, output.path, UInt32(RENAME_EXCL)) == 0
      else {
  ...
  private func subjects(from entries: [RecordingShieldEntry]) throws -> [RecordingShieldStatement
    .Subject]
  {
    var normalized = Set([RecordingShieldProfile.manifestFileName])
    var folded = Set([RecordingShieldProfile.manifestFileName])
    var subjects: [RecordingShieldStatement.Subject] = []
    for entry in entries {
      if RecordingShieldEntries.pathProblem(entry.path) {
        throw RecordingShieldSealingError.invalidPath(entry.path)
      }
      ...
      guard normalized.insert(nfc).inserted, folded.insert(fold).inserted else {
        throw RecordingShieldSealingError.pathConflict(entry.path)
      }
      switch entry.kind {
      case .regular:
