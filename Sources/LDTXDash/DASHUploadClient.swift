...
private let dashUploadLogger = Logger(
...
public enum DASHUploadError: Error, Equatable, LocalizedError {
  ...
  case missingManifestOrInitialization(
    objectName: String, byteCount: Int, statusCode: Int, body: Data)

  public var errorDescription: String? {
    switch self {
    case .nonHTTPResponse:
      "The DASH ingest server returned a non-HTTP response."
    case .rejected(let objectName, let byteCount, let statusCode, let body):
      "The DASH ingest server rejected \(objectName) (\(byteCount) bytes) with HTTP \(statusCode).\(Self.bodyDescription(body))"
    case .missingManifestOrInitialization(let objectName, let byteCount, let statusCode, let body):
...
public struct DASHRetryPolicy: Sendable, Equatable {
  ...
  public init(
    maxAttempts: Int = 3, retryStatusCodes: Set<Int> = [408, 425, 429, 500, 502, 503, 504]
  ) {
...
public struct DASHUploadClient: Sendable {
  private static let signpostLog = OSLog(
    subsystem: "tokyo.kaito.ldtx", category: "PointsOfInterest")
  ...
  private func put(
    ...
    putOnce(object) { result in
      ...
      case .failure(let error):
        dashUploadLogger.error(
          "DASH PUT transport failure object=\(object.name.rawValue, privacy: .public) bytes=\(object.data.count, privacy: .public) attempt=\(attempt, privacy: .public) elapsed=\(String(describing: elapsed), privacy: .public) error=\(error.localizedDescription, privacy: .public)"
        )
        completionHandler(.failure(error))
      case .success(let response):
        dashUploadLogger.notice(
          "DASH PUT response object=\(object.name.rawValue, privacy: .public) bytes=\(object.data.count, privacy: .public) attempt=\(attempt, privacy: .public) status=\(response.statusCode, privacy: .public) elapsed=\(String(describing: elapsed), privacy: .public)"
        )
