...
public final class MediaPipeSelfieSegmentationModel {
    ...
    private func predictSegmentation() -> SegmentationPrediction? {
        let signpostID = OSSignpostID(log: Self.signpostLog)
        os_signpost(.begin, log: Self.signpostLog, name: "Background Removal Prediction", signpostID: signpostID)
        defer {
            os_signpost(.end, log: Self.signpostLog, name: "Background Removal Prediction", signpostID: signpostID)
        }
        do {
...
private final class MetalBackgroundRemovalInferenceGate {
    ...
    init(device: MTLDevice, motionThreshold: Double) throws {
        ...
        guard let spatialAggregateFunction = library.makeFunction(
        ...
        guard let spatialSupportFunction = library.makeFunction(
    ...
    func shouldRunInference(
        ...
        guard let schedulingState = decisionState.withLockIfAvailable({ state in
        ...
        let referenceAggregateBuffer = bufferSelection.referenceIndex.map { aggregateBuffers[$0] }
    ...
    private func encodeMotionDecision(
        ...
        let threadgroupCount = (cellCount + aggregateSimdgroupsPerThreadgroup - 1) /
            aggregateSimdgroupsPerThreadgroup
