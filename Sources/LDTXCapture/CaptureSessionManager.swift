...
public enum CaptureSessionManagerError: Error, Equatable, LocalizedError {
  ...
  public var errorDescription: String? {
    ...
    case .cannotAddConnection(let key):
      "The capture connection could not be added: \(key)."
    case .missingInputPort(let key):
  ...
  @unchecked Sendable
{
  ...
  private func configureAndStart(
    ...
    os_signpost(
      .begin, log: Self.signpostLog, name: "Capture Session Start", signpostID: signpostID)
    defer {
      os_signpost(
        .end, log: Self.signpostLog, name: "Capture Session Start", signpostID: signpostID)
    }
  ...
  private func configureVideoInputs(
    ...
    for request in requests {
      ...
      guard let device = Self.allVideoDevices().first(where: { $0.uniqueID == request.deviceID })
      else {
      ...
      guard output.availableVideoPixelFormatTypes.contains(Self.preferredVideoPixelFormat) else {
        throw CaptureSessionManagerError.unsupportedVideoPixelFormat(
          Self.fourCC(Self.preferredVideoPixelFormat))
      }
  ...
  private func configureFormat(
    ...
    let requestedDuration =
      selectedFormat
      .flatMap {
        Self.nearestFrameRateRange($0, requestedFrameRate: requested).range?.minFrameDuration
      }
      ?? fallbackDuration
    ...
    try AVCaptureDeviceConfigurationGate.withLock {
      ...
      let nearestActiveFrameRate = Self.nearestFrameRateRange(
        camera.activeFormat, requestedFrameRate: requested)
  ...
  private static func bestFormat(
    ...
    camera.formats
      .filter {
        format(
          $0, matchesWidth: targetWidth, height: targetHeight,
          pixelFormat: preferredVideoPixelFormat)
      }
      .min { lhs, rhs in
  ...
  private static func nearestFrameRateRange(
    ...
    let candidates = format.videoSupportedFrameRateRanges.map { range in
      ...
      return (
        range: range, frameRate: selectedFrameRate,
        distance: abs(selectedFrameRate - requestedFrameRate)
      )
    }
  ...
  private static func activeVideoConfigurationSummary(for device: AVCaptureDevice) -> String {
    ...
    let activeRanges = activeFormat.videoSupportedFrameRateRanges
      ...
      .joined(separator: ",")
    return
      "\(Int(activeDimensions.width))x\(Int(activeDimensions.height)), pixelFormat=\(fourCC(activePixelFormat)), frameRates=[\(activeRanges)], activeMinFPS=\(frameRateDescription(for: device.activeVideoMinFrameDuration)), activeMaxFPS=\(frameRateDescription(for: device.activeVideoMaxFrameDuration))"
  }
