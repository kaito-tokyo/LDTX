...
enum SharedCaptureSessionPlanner {
  static func makePlans(
    ...
    let subscriptionsByID = Dictionary(
      uniqueKeysWithValues: expandedSubscriptions.map { ($0.id, $0) })
...
final class SharedCaptureSessionRegistry: @unchecked Sendable {
  ...
  private func reconcileSessions(
    ...
    os_signpost(
      .begin, log: Self.signpostLog, name: "Shared Capture Reconcile", signpostID: signpostID)
    let finish: @Sendable (Result<Void, any Error>) -> Void = { result in
      os_signpost(
        .end, log: Self.signpostLog, name: "Shared Capture Reconcile", signpostID: signpostID)
  ...
  private func reconcilePlans(
    ...
    let handlersByInterest = plan.subscriptionRoutes.reduce(
      into: [SharedCaptureSessionRouteInterest: [SampleHandler]]()
    ) { result, entry in
  ...
  fileprivate static func describeDemand(_ demand: SharedCaptureSessionSubscriptionDemand) -> String
  {
    ...
    if let video = demand.video {
      videoDescription =
        "\(video.deviceID)@\(video.targetWidth)x\(video.targetHeight)/\(video.frameRate)"
    } else {
      videoDescription = "-"
    }
...
private final class SharedCaptureSession: @unchecked Sendable {
  ...
  func reconcile(
    ...
    let signpostID = OSSignpostID(log: Self.signpostLog)
    os_signpost(
      .begin, log: Self.signpostLog, name: "Shared Capture Session Reconcile",
      signpostID: signpostID)
    defer {
      os_signpost(
        .end, log: Self.signpostLog, name: "Shared Capture Session Reconcile",
        signpostID: signpostID)
    }

    let previousState = lock.withLock {
      () -> (
        CaptureSessionRequest?, [SharedCaptureSessionRouteInterest: [SampleHandler]],
        [UUID: Set<SharedCaptureSessionRouteInterest>], [UUID: FailureHandler]
      ) in
      let state = (
        self.request, self.handlersByInterest, self.failureRoutesBySubscriptionID,
        self.failureHandlersBySubscriptionID
      )
