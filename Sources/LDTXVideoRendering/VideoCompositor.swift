...
public enum VideoCompositorError: Error, LocalizedError {
  ...
  public var errorDescription: String? {
    ...
    case .pixelBufferPoolPrewarmFailed(let status):
      "CVPixelBufferPool prewarm failed with status \(status)."
    case .outputPixelBufferCreationFailed(let status):
      "CVPixelBufferPoolCreatePixelBuffer failed with status \(status)."
    case .unsupportedPixelBufferFormat(let pixelFormat):
...
public final class VideoCompositor: @unchecked Sendable {
  ...
  public init(
    ...
    guard configuration.width > 0,
      ...
      configuration.pixelBufferPoolMinimumBufferCount > 0
    else {
    ...
    self.outputCanvasResourceManager =
      outputCanvasResourceManager
      ?? OutputCanvasResourceManager(
    ...
    do {
      ...
      guard let clearLuma = library.makeFunction(name: "clearLumaKernel"),
        ...
        let retainedTextureChromaAlpha = library.makeFunction(
          name: "retainedTextureChromaAlphaKernel"),
        let testPatternLuma = library.makeFunction(name: "testPatternLumaKernel"),
        let testPatternChroma = library.makeFunction(name: "testPatternChromaKernel")
      else {
        throw VideoCompositorError.shaderCompilationFailed(
          "Required kernel functions were not found.")
      }
      ...
      solidColorLumaAlphaPipeline = try device.makeComputePipelineState(
        function: solidColorLumaAlpha)
      solidColorChromaPipeline = try device.makeComputePipelineState(function: solidColorChroma)
      solidColorChromaAlphaPipeline = try device.makeComputePipelineState(
        function: solidColorChromaAlpha)
      linearGradientLumaPipeline = try device.makeComputePipelineState(function: linearGradientLuma)
      linearGradientChromaPipeline = try device.makeComputePipelineState(
        function: linearGradientChroma)
      radialGradientLumaPipeline = try device.makeComputePipelineState(function: radialGradientLuma)
      radialGradientChromaPipeline = try device.makeComputePipelineState(
        function: radialGradientChroma)
      conicGradientLumaPipeline = try device.makeComputePipelineState(function: conicGradientLuma)
      conicGradientChromaPipeline = try device.makeComputePipelineState(
        function: conicGradientChroma)
      retainedTextureLumaPipeline = try device.makeComputePipelineState(
        function: retainedTextureLuma)
      retainedTextureLumaAlphaPipeline = try device.makeComputePipelineState(
        function: retainedTextureLumaAlpha)
      retainedTextureChromaPipeline = try device.makeComputePipelineState(
        function: retainedTextureChroma)
      retainedTextureChromaAlphaPipeline = try device.makeComputePipelineState(
        function: retainedTextureChromaAlpha)
  ...
  public func render(_ components: [any MetalVideoComponent], into outputPixelBuffer: CVPixelBuffer)
    throws
  {
  ...
  public func renderCommands(
    _ components: [MetalVideoComponentCommand], into outputPixelBuffer: CVPixelBuffer
  ) throws {
  ...
  private func renderCommands(
    ...
    let outputTextures = try outputTextures(
      for: outputPixelBuffer, reusingOutputTextures: reusingOutputTextures)
    ...
    dispatch(
      encoder: clearEncoder, pipeline: clearLumaPipeline, width: configuration.width,
      height: configuration.height)
    clearEncoder.setComputePipelineState(clearChromaPipeline)
    dispatch(
      encoder: clearEncoder, pipeline: clearChromaPipeline, width: configuration.width / 2,
      ...
      do {
        ...
        case .solidColor(let component):
          ...
          if yWidth > 0 && yHeight > 0 {
            ...
            encoder.setBytes(
              &offsetXY, length: MemoryLayout<SIMD2<UInt32>>.stride,
              index: SolidColorArgumentIndex.offsetXY)
            encoder.setBytes(
              &luma0, length: MemoryLayout<UInt32>.stride, index: SolidColorArgumentIndex.luma0)
            if alpha0 != 255 {
              encoder.setBytes(
                &alpha0, length: MemoryLayout<UInt32>.stride, index: SolidColorArgumentIndex.alpha0)
            }
          ...
          if chromaWidth > 0 && chromaHeight > 0 {
            ...
            encoder.setComputePipelineState(pipeline)
            encoder.setBytes(
              &chromaOffsetXY, length: MemoryLayout<SIMD2<UInt32>>.stride,
              index: SolidColorArgumentIndex.offsetXY)
            encoder.setBytes(
              &chroma0, length: MemoryLayout<SIMD2<UInt32>>.stride,
              index: SolidColorArgumentIndex.chroma0)
            if alpha0 != 255 {
              encoder.setBytes(
                &alpha0, length: MemoryLayout<UInt32>.stride, index: SolidColorArgumentIndex.alpha0)
            }
        ...
        case .linearGradient(let component):
          ...
          bindOutput(outputY: outputLuma, outputUV: outputChroma, to: encoder)
          encoder.setBytes(
            &pointUV0, length: MemoryLayout<SIMD2<Float>>.stride,
            index: LinearGradientArgumentIndex.pointUV0)
          encoder.setBytes(
            &axisUV, length: MemoryLayout<SIMD2<Float>>.stride,
            index: LinearGradientArgumentIndex.axisUV)
          if yWidth > 0 && yHeight > 0 {
            encoder.setComputePipelineState(linearGradientLumaPipeline)
            encoder.setBytes(
              &offsetXY, length: MemoryLayout<SIMD2<UInt32>>.stride,
              index: LinearGradientArgumentIndex.offsetXY)
            encoder.setBytes(
              &luma0, length: MemoryLayout<Float16>.stride, index: LinearGradientArgumentIndex.luma0
            )
            encoder.setBytes(
              &alpha0, length: MemoryLayout<Float16>.stride,
              index: LinearGradientArgumentIndex.alpha0)
            encoder.setBytes(
              &luma1, length: MemoryLayout<Float16>.stride, index: LinearGradientArgumentIndex.luma1
            )
            encoder.setBytes(
              &alpha1, length: MemoryLayout<Float16>.stride,
              index: LinearGradientArgumentIndex.alpha1)
            dispatch(
              encoder: encoder, pipeline: linearGradientLumaPipeline, width: yWidth, height: yHeight
            )
          }
          if chromaWidth > 0 && chromaHeight > 0 {
            ...
            encoder.setBytes(
              ...
              index: LinearGradientArgumentIndex.offsetXY)
            encoder.setBytes(
              &chroma0, length: MemoryLayout<SIMD2<Float16>>.stride,
              index: LinearGradientArgumentIndex.chroma0)
            encoder.setBytes(
              &alpha0, length: MemoryLayout<Float16>.stride,
              index: LinearGradientArgumentIndex.alpha0)
            encoder.setBytes(
              &chroma1, length: MemoryLayout<SIMD2<Float16>>.stride,
              index: LinearGradientArgumentIndex.chroma1)
            encoder.setBytes(
              &alpha1, length: MemoryLayout<Float16>.stride,
              index: LinearGradientArgumentIndex.alpha1)
            dispatch(
              encoder: encoder, pipeline: linearGradientChromaPipeline, width: chromaWidth,
              height: chromaHeight)
          }
        case .radialGradient(let component):
          ...
          bindOutput(outputY: outputLuma, outputUV: outputChroma, to: encoder)
          encoder.setBytes(
            &centerUV, length: MemoryLayout<SIMD2<Float>>.stride,
            index: RadialGradientArgumentIndex.centerUV)
          encoder.setBytes(
            &radiusUV, length: MemoryLayout<SIMD2<Float>>.stride,
            index: RadialGradientArgumentIndex.radiusUV)
          if yWidth > 0 && yHeight > 0 {
            encoder.setComputePipelineState(radialGradientLumaPipeline)
            encoder.setBytes(
              &offsetXY, length: MemoryLayout<SIMD2<UInt32>>.stride,
              index: RadialGradientArgumentIndex.offsetXY)
            encoder.setBytes(
              &luma0, length: MemoryLayout<Float16>.stride, index: RadialGradientArgumentIndex.luma0
            )
            encoder.setBytes(
              &alpha0, length: MemoryLayout<Float16>.stride,
              index: RadialGradientArgumentIndex.alpha0)
            encoder.setBytes(
              &luma1, length: MemoryLayout<Float16>.stride, index: RadialGradientArgumentIndex.luma1
            )
            encoder.setBytes(
              &alpha1, length: MemoryLayout<Float16>.stride,
              index: RadialGradientArgumentIndex.alpha1)
            dispatch(
              encoder: encoder, pipeline: radialGradientLumaPipeline, width: yWidth, height: yHeight
            )
          }
          if chromaWidth > 0 && chromaHeight > 0 {
            ...
            encoder.setBytes(
              &chroma0, length: MemoryLayout<SIMD2<Float16>>.stride,
              index: RadialGradientArgumentIndex.chroma0)
            encoder.setBytes(
              &alpha0, length: MemoryLayout<Float16>.stride,
              index: RadialGradientArgumentIndex.alpha0)
            encoder.setBytes(
              &chroma1, length: MemoryLayout<SIMD2<Float16>>.stride,
              index: RadialGradientArgumentIndex.chroma1)
            encoder.setBytes(
              &alpha1, length: MemoryLayout<Float16>.stride,
              index: RadialGradientArgumentIndex.alpha1)
            dispatch(
              encoder: encoder, pipeline: radialGradientChromaPipeline, width: chromaWidth,
              height: chromaHeight)
          }
        case .conicGradient(let component):
          let startYUVColor = Self.fullRangeYUVColor(from: component.startColor)
          let endYUVColor = Self.fullRangeYUVColor(from: component.endColor)
          let bounds = component.destinationRect
          var offsetXY = SIMD2<UInt32>(bounds.x, bounds.y)
          ...
          encoder.setBytes(
            &centerUV, length: MemoryLayout<SIMD2<Float>>.stride,
            index: ConicGradientArgumentIndex.centerUV)
          encoder.setBytes(
            &startAngleRadians, length: MemoryLayout<Float>.stride,
            index: ConicGradientArgumentIndex.startAngleRadians)
          if yWidth > 0 && yHeight > 0 {
            encoder.setComputePipelineState(conicGradientLumaPipeline)
            encoder.setBytes(
              &offsetXY, length: MemoryLayout<SIMD2<UInt32>>.stride,
              index: ConicGradientArgumentIndex.offsetXY)
            encoder.setBytes(
              &luma0, length: MemoryLayout<Float16>.stride, index: ConicGradientArgumentIndex.luma0)
            encoder.setBytes(
              &alpha0, length: MemoryLayout<Float16>.stride,
              index: ConicGradientArgumentIndex.alpha0)
            encoder.setBytes(
              &luma1, length: MemoryLayout<Float16>.stride, index: ConicGradientArgumentIndex.luma1)
            encoder.setBytes(
              &alpha1, length: MemoryLayout<Float16>.stride,
              index: ConicGradientArgumentIndex.alpha1)
            dispatch(
              encoder: encoder, pipeline: conicGradientLumaPipeline, width: yWidth, height: yHeight)
          }
          if chromaWidth > 0 && chromaHeight > 0 {
            ...
            encoder.setBytes(
              &chroma0, length: MemoryLayout<SIMD2<Float16>>.stride,
              index: ConicGradientArgumentIndex.chroma0)
            encoder.setBytes(
              &alpha0, length: MemoryLayout<Float16>.stride,
              index: ConicGradientArgumentIndex.alpha0)
            encoder.setBytes(
              &chroma1, length: MemoryLayout<SIMD2<Float16>>.stride,
              index: ConicGradientArgumentIndex.chroma1)
            encoder.setBytes(
              &alpha1, length: MemoryLayout<Float16>.stride,
              index: ConicGradientArgumentIndex.alpha1)
            dispatch(
              encoder: encoder, pipeline: conicGradientChromaPipeline, width: chromaWidth,
              height: chromaHeight)
          }
        case .cameraInput(let component):
          ...
          if yWidth > 0 && yHeight > 0 {
            ...
            encoder.setBytes(
              &offsetXY, length: MemoryLayout<SIMD2<UInt32>>.stride,
              index: InputNv12DeviceArgumentIndex.offsetXY)
            dispatch(
              encoder: encoder, pipeline: inputNv12DevicePipelines.luma, width: yWidth,
              height: yHeight)
          }
          if chromaWidth > 0 && chromaHeight > 0 {
            ...
            encoder.setBytes(
              ...
              index: InputNv12DeviceArgumentIndex.offsetXY)
            dispatch(
              encoder: encoder, pipeline: inputNv12DevicePipelines.chroma, width: chromaWidth,
              height: chromaHeight)
          }
        case .retainedTexture(let component):
          guard component.colorTexture.pixelFormat == .b5g6r5Unorm,
            ...
            component.colorTexture.device.registryID == device.registryID
          else {
            throw VideoCompositorError.invalidConfiguration
          }
          if let alphaTexture = component.alphaTexture {
            guard alphaTexture.pixelFormat == .r8Unorm,
              ...
              alphaTexture.device.registryID == device.registryID
            else {
              throw VideoCompositorError.invalidConfiguration
            }
          }
        ...
        case .testPattern(let component):
          let bounds = component.destinationRect
          var offsetXY = SIMD2<UInt32>(bounds.x, bounds.y)
          let yWidth = Int(bounds.z - bounds.x)
          let yHeight = Int(bounds.w - bounds.y)
          ...
          if yWidth > 0 && yHeight > 0 {
            ...
            encoder.setBytes(
              &offsetXY, length: MemoryLayout<SIMD2<UInt32>>.stride,
              index: TestPatternArgumentIndex.offsetXY)
            encoder.setBytes(
              &timeSeconds, length: MemoryLayout<Float>.stride,
              index: TestPatternArgumentIndex.timeSeconds)
            dispatch(
              encoder: encoder, pipeline: testPatternLumaPipeline, width: yWidth, height: yHeight)
          }
          if chromaWidth > 0 && chromaHeight > 0 {
            ...
            encoder.setBytes(
              ...
              index: TestPatternArgumentIndex.timeSeconds)
            dispatch(
              encoder: encoder, pipeline: testPatternChromaPipeline, width: chromaWidth,
              height: chromaHeight)
          }
        }
        encoder.endEncoding()
  ...
  private func makeBoundSource(
    ...
    guard let lumaTexture = CVMetalTextureGetTexture(lumaMetalTexture),
      let chromaTexture = CVMetalTextureGetTexture(chromaMetalTexture)
    else {
  ...
  private func makeOutputTextures(for pixelBuffer: CVPixelBuffer) throws -> MadeOutputTextures {
    ...
    guard let luma = CVMetalTextureGetTexture(lumaMetalTexture),
      let chroma = CVMetalTextureGetTexture(chromaMetalTexture)
    else {
      throw VideoCompositorError.textureCreationFailed(kCVReturnInvalidArgument)
    }
  ...
  private func bindOutput(
    outputY: MTLTexture, outputUV: MTLTexture, to encoder: MTLComputeCommandEncoder
  ) {
  ...
  private func dispatch(encoder: MTLComputeCommandEncoder, pipeline: MTLComputePipelineState) {
    dispatch(
      encoder: encoder, pipeline: pipeline, width: configuration.width, height: configuration.height
    )
  }
  ...
  public static func makePreviewNV12ToBGRAPipeline(device: MTLDevice) throws
  ...
  public static func makePreviewLumaToGrayscaleBGRAPipeline(device: MTLDevice) throws
    -> MTLComputePipelineState
  {
    let library = try makeShaderLibrary(device: device)
    guard let function = library.makeFunction(name: "previewLumaToGrayscaleBGRAKernel") else {
      throw VideoCompositorError.shaderCompilationFailed(
        "previewLumaToGrayscaleBGRAKernel was not found.")
    }
    return try device.makeComputePipelineState(function: function)
  }
