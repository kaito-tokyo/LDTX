...
private final class InputDeviceAudioRecordingSession: @unchecked Sendable {
  ...
  func configure(
    ...
    guard request.protocolVersion == LDTXRecordingWriterXPCInterfaces.protocolVersion else {
      throw InputDeviceAudioRecordingServiceError.incompatibleProtocol(request.protocolVersion)
    }
    guard request.trackID.hasPrefix("input-device-audio:") else {
      throw InputDeviceAudioRecordingServiceError.invalidTrackID(request.trackID)
    }
    ...
    while let data = lock.withLock({ consumer?.read() }) {
      ...
      case .buffer(let buffer):
        ...
        guard let format = values.0, let writer = values.1 else {
          throw InputDeviceAudioRecordingServiceError.missingFormat
        }
        writer.append(
  ...
  private func commit(_ segment: SegmentedMP4Segment) {
    do {
      let committed = try lock.withLock { () -> (UInt64, Ldtx_Recording_Xpc_V1_Context) in
        guard let output else { throw InputDeviceAudioRecordingServiceError.notConfigured }
...
private enum InputDeviceAudioRecordingServiceError: Error {
...
private final class InputDeviceAudioRecordingConnection: NSObject, LDTXRecordingWriterServiceXPC,
  @unchecked Sendable
{
  private let session: InputDeviceAudioRecordingSession

  init(connection: NSXPCConnection) {
    session = InputDeviceAudioRecordingSession(connection: connection)
  }
  ...
  func finish(_ request: Data, withReply reply: @escaping (Data) -> Void) {
    let reply = InputDeviceAudioRecordingReply(reply)
...
private final class InputDeviceAudioRecordingReply: @unchecked Sendable {
...
private final class InputDeviceAudioRecordingListenerDelegate: NSObject, NSXPCListenerDelegate {
  func listener(
    ...
    connection.exportedObject = InputDeviceAudioRecordingConnection(connection: connection)
    connection.resume()
    return true
  }
}
...
private let delegate = InputDeviceAudioRecordingListenerDelegate()
