...
public enum H264VideoEncoderError: Error, LocalizedError {
  ...
  public var errorDescription: String? {
    ...
    case .unexpectedBitstreamProfile(let codec):
      "The H.264 encoder produced \(codec), not required H.264 High@L4.2."
    case .invalidPixelBuffer:
      "The H.264 encoder requires a matching NV12 full-range pixel buffer."
    }
  }
}
...
public final class H264VideoEncoder: @unchecked Sendable {
  ...
  private func didEncode(
    ...
    do {
      let codec = try Self.codecString(from: sampleBuffer)
      guard Self.isHigh42CodecString(codec) else {
        outputHandler(
          .failure(H264VideoEncoderError.unexpectedBitstreamProfile(codec)))
        return
      }
    } catch {
  ...
  static func isHigh42CodecString(_ codec: String) -> Bool {
    guard codec.count == 11, codec.hasPrefix("avc1."),
      let profile = UInt8(codec.dropFirst(5).prefix(2), radix: 16),
      let level = UInt8(codec.suffix(2), radix: 16)
    else { return false }
    return profile == 0x64 && level == 0x2A
  }
