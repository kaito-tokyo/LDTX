...
struct VideoLayersDetailPane: View {
  ...
  var workspaceVideoComponents: [ProgramVideoComponentRecord]
  ...
  private var availableWorkspaceVideoComponents: [ProgramVideoComponentRecord] {
    let usedNames = Set(videoLayers.map(\.componentName))
    return workspaceVideoComponents.filter { !usedNames.contains($0.name) }
  }

  private var availableVideoInputDevices: [ProgramInputDeviceRecord] {
    let usedNames = Set(videoLayers.map(\.componentName))
  ...
  private func addWorkspaceVideoComponent(_ resource: ProgramVideoComponentRecord) {
  ...
  private func addVideoInputDevice(_ device: ProgramInputDeviceRecord) {
  ...
  private func applyVideoLayerPreferencesToWorkingComposite() {
    compositeProgramDefinition = ProgramVideoComponentResolver.applying(
...
enum VideoLayerDestinationPolicy {
  static func supportsDestination(
    ...
    videoComponents: [ProgramVideoComponentRecord]
  ) -> Bool {
...
extension ProgramComponentDefinition {
  ...
  @MainActor
  private var videoLayerPreviewComponents: [ProgramVideoComponentRecord] {
    [
      ProgramVideoComponentRecord(
        name: "Clock",
