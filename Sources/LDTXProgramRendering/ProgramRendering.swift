...
extension CompositeProgramDefinition {
  public func appendComponentCommands(
  ...
  public func components(
    ...
    for step in steps.reversed() {
      let stepComponents = step.component.components(
        ...
        colorRangeOverride: colorRangesByInputKey[inputCameraDeviceMappingKey(for: step)]
          ?? .unspecified,
        timeSeconds: timeSeconds
      )
...
extension ClockComponent {
  public func destinationRect(outputWidth: Int, outputHeight: Int) -> SIMD4<UInt32> {
...
extension FillClip {
  fileprivate func rect(width canvasWidth: Int, height canvasHeight: Int) -> SIMD4<UInt32> {
  ...
  fileprivate func clippedPixel(_ value: Float, limit: Int) -> Int {
    min(max(Int(value.rounded(.down)), 0), limit)
  }
}

extension InputDeviceComponent {
  fileprivate func sourceRect(width: Int, height: Int) -> SIMD4<Float> {
  ...
  fileprivate func destinationRect(width: Int, height: Int) -> SIMD4<UInt32> {
    let canvasWidth = Float(max(width, 1))
    let canvasHeight = Float(max(height, 1))
  ...
  fileprivate func sourceUVRect() -> SIMD4<Float> {
  ...
  fileprivate func unclippedDestinationPixelRect(
    canvasWidth: Float,
    canvasHeight: Float
  ) -> SIMD4<Float> {
    let source = sourceUVRect()
  ...
  fileprivate func clippedDestinationPixelRect(
    canvasWidth: Float,
    canvasHeight: Float
  ) -> SIMD4<Float> {
  ...
  fileprivate func visibleDestinationFractions(
    canvasWidth: Float,
    canvasHeight: Float
  ) -> SIMD4<Float> {
  ...
  fileprivate func percentToUnit(_ value: Float) -> Float {
