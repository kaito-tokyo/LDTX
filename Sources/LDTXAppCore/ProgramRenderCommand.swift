...
struct ProgramRenderCommand {
  ...
  func run() async throws {
    ...
    do {
      ...
      for frameIndex in 0..<frameCount {
        let timeSeconds =
          startTimeSeconds + program.canvas.frameRate.secondsPerFrame * Float(frameIndex)
    ...
    for key in ["--input-camera", "--input-mapping"] {
      if let value = options[key] {
        for pair in value.split(separator: ",") {
          ...
          let cameraQuery = pair[pair.index(after: equalsIndex)...].trimmingCharacters(
  ...
  private func inputProgramData() throws -> Data {
    guard let path = options["--program-json"] ?? options["--input"] ?? positionalArguments.first
    else {
  ...
  private func outputImageDestination() throws -> ProgramRenderOutput {
    ...
    let format =
      try outputFormatOption() ?? ProgramRenderImageFormat(pathExtension: url.pathExtension)
  ...
  private func sandboxMoviesDirectory() throws -> URL {
    guard
      let moviesDirectory = FileManager.default.urls(
        for: .moviesDirectory,
        in: .userDomainMask
      ).first
    else {
  ...
  private func makeImageData(pixelBuffer: CVPixelBuffer, format: ProgramRenderImageFormat) throws
    -> Data
  {
  ...
  static func copyPixelBuffer(_ source: CVPixelBuffer, to destination: CVPixelBuffer) {
    ...
    if planeCount == 0 {
      ...
      copyRows(
        ...
        bytesPerRow: min(
          CVPixelBufferGetBytesPerRow(source), CVPixelBufferGetBytesPerRow(destination)),
    ...
    for planeIndex in 0..<planeCount {
      guard let sourceBaseAddress = CVPixelBufferGetBaseAddressOfPlane(source, planeIndex),
        let destinationBaseAddress = CVPixelBufferGetBaseAddressOfPlane(destination, planeIndex)
      else {
        continue
      }
  ...
  func copyAndYield(_ sourcePixelBuffer: CVPixelBuffer) {
    ...
    guard
      ...
      CVPixelBufferGetWidth(sourcePixelBuffer)
        == CVPixelBufferGetWidth(pixelBuffers[nextPixelBufferIndex]),
      CVPixelBufferGetHeight(sourcePixelBuffer)
        == CVPixelBufferGetHeight(pixelBuffers[nextPixelBufferIndex])
    else {
      return
    }
...
private final class ProgramRenderCameraFrameProvider {
  ...
  func start() async throws {
    let uniqueCameras = Dictionary(grouping: mappings, by: { $0.camera.id }).compactMap {
      $0.value.first?.camera
    }
...
private enum ProgramRenderOutput {
  ...
  var format: ProgramRenderImageFormat {
    switch self {
    case .file(_, let format), .standardOutput(let format):
      format
    }
  }
...
private enum ProgramRenderCommandError: LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case .cameraNotFound(let query):
      "No camera matched input mapping value: \(query)."
    case .cameraFrameTimedOut(let name):
