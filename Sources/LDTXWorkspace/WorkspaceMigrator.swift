...
public enum WorkspaceMigrator {
  ...
  static func migrate(
    ...
    if protobufMigration.sourceFormatVersion == 0 {
      for (inputDeviceID, physicalDeviceID) in protobufMigration
        .legacyPhysicalDeviceIDsByInputDeviceID
    ...
    if protobufMigration.sourceFormatVersion < 2 {
      migrateLegacyVideoMute(
        definition: migratedDefinition,
        preferences: &migratedPreferences.programPreferences
      )
    }
  ...
  private static func migrateLegacyVideoLayers(
    ...
    where preferences.videoLayersByProgramName[definition.programs[programIndex].name] == nil {
      ...
      let layers = definition.programs[programIndex].composite.steps.map {
        step -> VideoLayerPreference in
  ...
  private static func migrateLegacyVideoMute(
    definition: WorkspaceDefinition,
    preferences: inout ProgramPreferences
  ) {
    var sharedInputDeviceIDs: [String: String] = [:]
    for component in definition.videoComponents where sharedInputDeviceIDs[component.name] == nil {
      sharedInputDeviceIDs[component.name] = component.inputDeviceID
    }

    for program in definition.programs {
      var directInputDeviceIDs: [String: String] = [:]
      for step in program.composite.steps where directInputDeviceIDs[step.name] == nil {
        guard case .inputCameraDevice(let component) = step.component else { continue }
        directInputDeviceIDs[step.name] = component.inputDeviceID
      }
      var layers = preferences.videoLayers(forProgramNamed: program.name)
      for index in layers.indices {
        guard
          let inputDeviceID = directInputDeviceIDs[layers[index].componentName]
            ?? sharedInputDeviceIDs[layers[index].componentName]
        else { continue }
        layers[index].isMuted = preferences.isVideoMuted(inputDeviceName: inputDeviceID)
      }
      preferences.setVideoLayers(layers, forProgramNamed: program.name)
    }
    preferences.videoMutedByInputDeviceName.removeAll()
  }
