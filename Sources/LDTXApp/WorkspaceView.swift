...
struct WorkspaceView: View {
    ...
    var body: some View {
        ...
        .onChange(of: workspaceStore.definition.inputDevices) { _, _ in
            restartAudioMonitor()
        }
        .onDisappear {
    ...
    private func configureAutomationHandlers() {
        automationState.updateHandlers(AppAutomationHandlers(
            ...
            selectProgram: { name, isScratchPad in
                if isScratchPad {
                    return AppAutomationCommandResult(ok: false, message: "Only saved Programs can be selected.")
                }
    ...
    private func deleteWorkspaceInputDevice(id: String) {
        ...
        if mainWindowState.selectedWorkspaceInputDeviceID == id {
            mainWindowState.selectedWorkspaceInputDeviceID = workspaceStore.definition.inputDevices.first?.id
        }

        compositeProgramDefinition = compositeClearingInputDeviceReference(id, in: compositeProgramDefinition)
    ...
    private func compositeClearingInputDeviceReference(
        ...
        for stepIndex in updated.steps.indices {
            guard case var .inputCameraDevice(payload) = updated.steps[stepIndex].component,
                  payload.inputDeviceID == inputDeviceID else {
                continue
            }
        ...
        for channelIndex in updated.audioChannels.indices {
            guard case var .inputAudioDevice(payload) = updated.audioChannels[channelIndex].component,
                  payload.inputDeviceID == inputDeviceID else {
                continue
            }
            payload.inputDeviceID = nil
