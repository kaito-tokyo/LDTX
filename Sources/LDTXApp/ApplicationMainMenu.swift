...
final class ApplicationMainMenu: NSMenu {
  ...
  private let settingsItem = NSMenuItem(title: "Settings…", action: #selector(ApplicationRouter.showSettings), keyEquivalent: ",")
  private let newWorkspaceItem = NSMenuItem(title: "New Workspace", action: #selector(ApplicationRouter.newWorkspace), keyEquivalent: "n")
  private let openFileItem = NSMenuItem(title: "Open File…", action: #selector(ApplicationRouter.openFile), keyEquivalent: "o")
  private let saveItem = NSMenuItem(title: "Save", action: #selector(ApplicationRouter.save), keyEquivalent: "s")
  private let saveAsItem = NSMenuItem(title: "Save As…", action: #selector(ApplicationRouter.saveAs), keyEquivalent: "S")
  private let reloadItem = NSMenuItem(title: "Reload Workspace", action: #selector(ApplicationRouter.reload), keyEquivalent: "")
  private let inspectorItem = NSMenuItem(title: "Toggle Inspector", action: #selector(WorkspaceV4WindowController.toggleInspector), keyEquivalent: "")
  private let crashReportsItem = NSMenuItem(title: "Show Crash Reports in Finder", action: #selector(ApplicationRouter.crashReports), keyEquivalent: "")
  ...
  private func buildApplicationMenu() {
    applicationMenu.addItem(NSMenuItem(title: "About LDTX", action: #selector(NSApplication.orderFrontStandardAboutPanel(_:)), keyEquivalent: ""))
    applicationMenu.addItem(settingsItem)
    applicationMenu.addItem(.separator())
    servicesMenuItem.submenu = NSMenu(title: "Services")
    applicationMenu.addItem(servicesMenuItem)
    applicationMenu.addItem(NSMenuItem(title: "Hide LDTX", action: #selector(NSApplication.hide(_:)), keyEquivalent: "h"))
    applicationMenu.addItem(NSMenuItem(title: "Show All", action: #selector(NSApplication.unhideAllApplications(_:)), keyEquivalent: ""))
    applicationMenu.addItem(.separator())
    applicationMenu.addItem(NSMenuItem(title: "Quit LDTX", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q"))
  }
