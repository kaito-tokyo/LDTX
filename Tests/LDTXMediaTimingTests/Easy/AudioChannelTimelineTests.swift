...
@Suite("LDTXMediaTimingEasyTests", .tags(.easy))
struct AudioChannelTimelineTests {
  @Test func frameIndexConversionCoversTimescalesRoundingAndNegativePTS() throws {
    ...
    for invalidTime in [
      CMTime.invalid, CMTime.indefinite, CMTime.positiveInfinity, CMTime.negativeInfinity,
    ] {
      #expect(throws: AudioChannelTimelineError.self) {
        try AudioChannelTimeline.frameIndex(for: invalidTime)
      }
    }
    #expect(throws: AudioChannelTimelineError.self) {
      try AudioChannelTimeline.frameIndex(for: .zero, sampleRate: 0)
    }
  }
