  ...
  struct H264VideoEncoderIntegrationTestSuite {
    ...
    @Test func testRecordingClockDiscardsEmittedHistoryWithoutChangingFutureMapping() throws {
      ...
      for batch in 0..<100 {
        for index in 0..<32 {
          ...
          _ = try clock.converterSample(
            makeAudioSample(startFrame: frame + 48_000, frameCount: 512))
        }
    ...
    @Test func testMonoRecordingWithSubsampleStartFinalizes() async throws {
      ...
      for segment in output.values {
        for box in try MP4TimingBox.parse(segment.data)
        where box.type == MP4TimingBox.fourCC("moof") {
