...
final class FileMP4WriterTests: XCTestCase {
  ...
  func stressSegmentedWriterWith1080p60VideoToolboxPassthrough() async throws {
    try LDTXTestConfiguration.skipUnlessHeavyMediaTestsEnabled(
      "1080p60 VideoToolbox segmented writer passthrough")
  ...
  func generateInspectableLocalDASHStream() async throws {
    try LDTXTestConfiguration.skipUnlessHeavyMediaTestsEnabled(
      "inspectable local DASH stream generation")
    ...
    for frameIndex in 0..<(configuration.frameRate * inputDurationSeconds) {
      ...
      while nextAudioSampleIndex < audioSampleCount,
        CMTime(
          ...
          timescale: CMTimeScale(configuration.audioSampleRate)) < videoEndTime
      {
  ...
  func testSegmentedWriterAllowsAudioToStartAfterFirstVideoPTS() async throws {
    try LDTXTestConfiguration.skipUnlessHeavyMediaTestsEnabled(
      "segmented writer delayed-audio start")
  ...
  func testSegmentedWriterFlushesPendingVideoWhenAudioStartsWriter() async throws {
    try LDTXTestConfiguration.skipUnlessHeavyMediaTestsEnabled(
      "segmented writer pending-video flush")
  ...
  func testSegmentedWriterAcceptsFloat32ProgramAudio() async throws {
    try LDTXTestConfiguration.skipUnlessHeavyMediaTestsEnabled(
      "segmented writer Float32 program audio")
  ...
  func testSegmentedWriterDropsRepeatedVideoPTS() async throws {
    try LDTXTestConfiguration.skipUnlessHeavyMediaTestsEnabled(
      "segmented writer repeated video PTS drop")
  ...
  func testSegmentedWriterAcceptsLargePTSFloat32ProgramAudio() async throws {
    try LDTXTestConfiguration.skipUnlessHeavyMediaTestsEnabled(
      "segmented writer large-PTS Float32 program audio")
  ...
  func testAudioSampleBufferNormalizerPreservesSourcePTS() throws {
    ...
    XCTAssertEqual(
      ...
      firstSource.presentationTimeStamp
    )
    ...
    XCTAssertEqual(
      ...
      secondSource.presentationTimeStamp
    )
  ...
  func testEncodedAudioVideoPTSIsMonotonicAcrossInputFormats() async throws {
    try LDTXTestConfiguration.skipUnlessHeavyMediaTestsEnabled(
      "formula-generated audio/video MP4 pattern matrix")
  ...
  func testExternalRecordingPTSIsMonotonic() async throws {
    guard let path = ProcessInfo.processInfo.environment["LDTX_EXTERNAL_RECORDING_PATH"] else {
      throw XCTSkip("Set LDTX_EXTERNAL_RECORDING_PATH to inspect a captured main.mp4")
    }
  ...
  private static func makePixelBuffer(
    ...
    case kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange,
      ...
      try fillBiPlanarYUV(
        unwrappedPixelBuffer, width: width, height: height, frameIndex: frameIndex)
    ...
    data.withUnsafeMutableBytes { rawBuffer in
      ...
      for frame in 0..<sampleCount {
        ...
        for channel in 0..<channelCount {
          ...
          let sweep = sin(
            2.0 * Double.pi * Double(absoluteSample % sampleRate) / Double(sampleRate))
          let value = (sin(phase) * 0.55 + sweep * 0.15) * Double(Int16.max)
          samples[frame * channelCount + channel] = Int16(
            max(Double(Int16.min), min(Double(Int16.max), value)))
        }
      }
    }
  ...
  private static func assertSamplePresentationTimesAreStrictlyIncreasing(
    ...
    while let sampleBuffer = output.copyNextSampleBuffer() {
      ...
      let presentationTime =
        outputPresentationTime.isValid
  ...
  private static func writerConfigurationDescription(
    _ configuration: SegmentedMP4WriterConfiguration
  ) -> String {
    [
      "width=\(configuration.width)",
      "height=\(configuration.height)",
...
extension Array where Element == SegmentedMP4Segment {
  fileprivate var mediaSegmentCount: Int {
    count { segment in
