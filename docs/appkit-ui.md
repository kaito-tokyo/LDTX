...
Run the `LDTXAppKitSystemTests` and `LDTXAppUITests` Xcode schemes for AppKit window, application lifecycle, and accessibility-driven UI regression tests. Generate project changes with XcodeGen. Use a worktree-specific DerivedData directory and run signed builds and tests outside the sandbox as required by AGENTS.md.
